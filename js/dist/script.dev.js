...
$(document).pjax('.nav-right nav a,.nav-left .avatar_target,.site_url', '.pjax', {
  fragment: '.pjax',
  timeout: 8000
});
$(document).on({
  /*点击链接后触发的事件*/
  'pjax:click': function pjaxClick() {
    /*原有内容淡出*/
...
function afterPjax() {
  ...
  content.css({
    'opacity': 1
  }).removeClass('fadeOuts').addClass('fadeIns');
  bind();
  /*discus获取评论数*/
...
$(".nav-left ul li>div").on("click", function (e) {
  ...
  var categories = $(this).data('rel').split('<--->'); // $('#default-panel > .right-title').text(categories[categories.length - 1]);
...
if (shortcutKey) {
  $(document).keydown(function (e) {
    ...
    // if (!$searchInput.is(":focus") && !$tagSearchInput.is(':focus') && !document.activeElement) {
      if (e.keyCode === 74) {
        /* J */
        container.animate({
  ...
  $(document).keyup(function (e) {
    if (!$searchInput.is(":focus") && !$tagSearchInput.is(':focus') && !$('#comments textarea').is(':focus')) {
      if (e.which === 83) {
        /* S - 显示/隐藏文章列表 */
        $fullBtn.trigger("click");
      } else if ((e.which === 73 || e.which === 105) && $(".nav").css('margin-left') === '0px' && !$('.title-list').hasClass('friend')) {
        /* I */
        inputChange();
      } else if (e.which === 87) {
        /* W - 切换大纲视图 */
...
function inputChange() {
  ...
  var categories = $(".nav-left ul li>div.active").data('rel').split('<--->'); // 处理特殊字符
  ...
  if (val === "") {
    ...
    searchType = '标题';
    containType = '包含'; // $(".nav-right nav a").css("display", "none");
...
$(function () {
  ...
  $('#search-panel > .icon-tag').on('click', function (e) {
    ...
    $(".nav-right .tags-list").css({
      'left': isPhone ? 'auto' : _offset.left - 20 + 'px',
  ...
  $('#search-panel > .icon-left').on('click', function () {
    ...
    $(this).parent().hide(); // $('#default-panel').show()
  ...
  $('#search-panel > .icon-file-tree').on('click', function (e) {
    $(this).parent().hide();
    $('#title-list-nav').hide();
    $('#tags-grid').hide();
  ...
  var showTag = false; // 显示标签

  $('#default-panel > .icon-tag').on('click', function (e) {
    e.stopPropagation();
  ...
  $('#tags-grid > .article-tag-list-item').on('click', function (e) {
    $searchInput.val("#" + $(this).text().trim()).change();
  }); // 回到默认面板

  $('#outline-panel > .icon-list').on('click', function (e) {
    $(this).parent().hide();
    $outlineList.hide(); // $('#default-panel').show()
...
function bind() {
  ...
  if ($('#theme_highlight_on').val() === 'true') {
    $('pre code').each(function (i, block) {
      ...
      var hasCopy = $('#theme_code_copy').val() !== 'false'; // 添加复制功能

      $(this).after('<div class="code-embed"><span class="code-embed-type">' + (codeClass.indexOf('hljs') === -1 ? codeClass : codeClass.indexOf('hljs') === 0 ? '' : codeClass.replace(/[\s]?hljs/g, '')) + '</span>' + (hasCopy ? '<span class="code-embed-copy" onclick="copyCode(this)">复制代码</span>' : '') + '</div>'); // 渲染样式
  ...
  $("#post .pjax .index h1:eq(0)").addClass("article-title"); //绑定文章内tag的搜索事件
  ...
  syncOutline(container[0]); //绑定文章toc的滚动事件
  ...
  $("a[href^='#']").click(function () {
    ...
    container.animate({
      scrollTop: container.scrollTop > targetOffsetTop ? targetOffsetTop + 20 : targetOffsetTop - 20
    }, 500, 'swing', function () {
      clickScrollTo = false;
    });
    return false;
  ...
  if ($("#comments").hasClass("disqus")) {
    ...
    $disqusCount.bind("DOMNodeInserted", function (e) {
      $(".count-comment").text($this.text().replace(/[^0-9]/ig, ""));
    });
  }
  /*给文章中的站内跳转绑定pjax*/
